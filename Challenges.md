...
and also supporting kubernetes as a cluster manager.
...
define the range of ports on the host...etc. We can also specify how many replicas we need for this Pod (Through Deployment directive), 
which kubernetes master node will make sure that the cluster
 stays always consistent by having these replicas up and running anywhere in the cluster.
 - The second problem that we need to overcome is the dynamic address mapping problem, 
 because these replicas run on different IP addresses and those Addresses could be changed in case of failures. Kubernetes overcome this by the concept of a SERVICE.
 I have overcome this problem in `Bioflows Distributed mode` by using `Consul` from hashicorp, which contains a distributed DNS. I designed each Bioflows node to register itself 
 in the DNS immediately after starting up 
